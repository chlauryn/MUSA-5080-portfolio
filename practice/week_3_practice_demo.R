...
my_variable <- "B07012_002"     # the variable code, in quotes
...
county_data %>%                    #DONT REASSIGN IT TO THE SAME NAME, IF NOT THE SLICE WILL BE PERMANENT!
  arrange(desc(moe_pct)) %>%
  slice_head(n = 15) %>% # NEED ARRANGE FIRST SO THEY KNOW TO GET THE TOP 15
  ggplot(aes(x = NAME, y = estimate)) +
  geom_col() +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe)) +
  coord_flip()
...
poverty_change <- bind_rows(
  get_poverty(2018),
  get_poverty(2023)
)                               #PUTS ROWS UNDERNEATH #LONG FORMAT


# Reformat to wide to put the years side by side
poverty_wide <- poverty_change %>%
  select(GEOID, NAME, period, estimate, moe) %>%     #first select the column names u want
  pivot_wider(
    names_from  = period,
    values_from = c(estimate, moe)
  )

# Compute change (for z-score 90% --> 1.645 is standard error value)
...
poverty_tested %>%
  ...
  geom_col(aes(fill = change > 0)) +                                  #geom_column, set condition for color to vary depending on whether change >/< 0
  geom_errorbar(aes(ymin = change - moe_diff,
                    ymax = change + moe_diff), width = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed") +                    # Add dashed line at zero
  coord_flip() +
  theme_minimal() +
...
# normal code, but can add emojis!!!! to a "dot"
